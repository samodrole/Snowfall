<script>
    import './styles.css'
    export let options;
    export let legend;
    export let userSelected = options[0].value;
		export let fontSize = 16;
		export let flexDirection = 'column'

    const uniqueID = `legend-${Math.floor(Math.random() * 100)}`
  
    const slugify = (str = "") =>
      str.toLowerCase().replace(/ /g, "-").replace(/\./g, "");
  </script>
  

<div role="radiogroup" 
				 class="group-container"
				 aria-labelledby={`label-${uniqueID}`}
				 style="font-size:{fontSize}px; flex-direction:{flexDirection}" 
				 id={`group-${uniqueID}`}>
        <div class="legend" 
						 id={`label-${uniqueID}`}>{legend}
				</div>
	<div class='options'>
		 {#each options as { value, label } (value)}
	<div class='option'>
		      <input
        class="sr-only"
        type="radio"
        id={`${uniqueID}-${value}`}
        bind:group={userSelected}
        value={value} />
      <label class='option' for={`${uniqueID}-${value}`}> {label} </label>
	</div>

    {/each}
   
	</div>
      
  </div>

  
  <style>
    .group-container {
      display: flex;
      flex-direction: row;
      margin-bottom: 24px;
    }


    .legend {
      margin-right: 0.5rem;
      margin-bottom: 12px;
      font-size: 1.2rem;
    line-height: 1.6;
    color: var(--textColorDark60);
    }

    label {
      user-select: none;
      line-height: 1.2em;
      font-weight: 400;
      color: var(--textColorDark40);
      min-width: 100px;
      max-width: 150px;
      text-align: center;
    }
  
    .sr-only {
      position: absolute;
      clip: rect(1px, 1px, 1px, 1px);
      padding: 0;
      border: 0;
      height: 1px;
      width: 1px;
      overflow: hidden;
    }
  
    input[type="radio"] {
      position: absolute;
    }
  
    input[type="radio"] + label {
      position: relative;
    }
		
		.options {
			display: flex;
		}
		
    .option > label{
        padding: 8px;
        line-height: 1.3em;
        appearance: none;
        background: none;
        font-weight: 600;
        font-size: 1.2rem;
        color: var(--textColorDark100);
        border: 1px solid var(--textColorDark100);
        border-radius: 0;
        outline: none;
        cursor: pointer;
    }
    @media (max-width: 600px) {
        .option > label{
            padding: 12px 6px;
            font-size: 1rem;
        }
    }
    .option + .option > label {
        border-left-width: 0;
    }
    .option:first-of-type > label {
        border-radius: 6px 0 0 6px;
    }
    .option:last-of-type > label {
        border-radius: 0 6px 6px 0;
    }
  
  
    input[type="radio"]:checked + label {
        background: var(--textColorDark100);
        color: white;
    }
  
    /* input[type="radio"]:focus + label {
        box-shadow: 0 0px 8px var(--textColorDark100);
    }   */
    
    input[type="radio"]:disabled + label {
      color: darken(var(--textColorDark40), 10);
    }
  
    input[type="radio"]:disabled + label {
      background: var(--textColorDark40);
    } 
    </style>