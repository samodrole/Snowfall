            ...
            {#if maxDaysSelcted === "Yes"}
                ...
                {#each location[1] as season}
                    <g class={filterBy === location[0] ? "bringForward" : ""} transform="translate({xScale(season["Season"])}, {yScale(season["Snowfall Days"])})">
                        <rect width={xScale.bandwidth()} height={innerHeight - yScale(season["Snowfall Days"])} x="0" y="0" fill={locationsMax[filterBy] === season["Snowfall Days"] ? colors(season.Location) : "var(--textColorDark5)"}/>
                        <text x={xScale.bandwidth()/2} dy="-0.8rem" text-anchor="middle" font-weight={locationsMax[filterBy] === season["Snowfall Days"] ? "700" : "500"} fill={locationsMax[filterBy] === season["Snowfall Days"] ? "var(--textColorDark100)" : "var(--textColorDark20)"}>{locationsMax[filterBy] === season["Snowfall Days"] ? season["Snowfall Days"] + " Days" : season["Snowfall Days"]}</text>
                    </g>
                {/each}
            {:else}
